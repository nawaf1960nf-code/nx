      ...
      <div className="mb-6">
        <div className="mb-3 flex items-center justify-between text-sm">
          <span className="font-semibold text-white">
            {t.exam.counter(index + 1, total)}
          </span>
          <span
            className="rounded-full px-2.5 py-0.5 text-xs font-medium"
            style={{ background: `${accent}1f`, color: accent }}
          >
            {t.exam.chShort} {question.chapter} · {t.qType[question.type] ?? question.type}
          </span>
        </div>
          ...
          {question.options.map((opt, i) => {
            ...
            return (
              <motion.button
                ...
                className="flex w-full items-center gap-3 rounded-2xl border p-4 text-start text-sm transition-all duration-200 disabled:cursor-default"
                style={{
