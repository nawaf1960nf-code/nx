...
export function AIBadge() {
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-brand-100/80"
      style={{ color: enabled ? "#34d399" : "#818cf8" }}
    >
      <Sparkles className="h-3.5 w-3.5" />
      <span className="text-brand-100/80">
        {enabled ? "AI Engine: Live" : "AI Engine: Smart Offline Mode"}
      </span>
    </span>
  );
}
